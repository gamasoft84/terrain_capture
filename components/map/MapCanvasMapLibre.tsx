...
import { ESRI_TILE_TEMPLATE } from "@/lib/map/esri-tiles";
import {
  useCallback,
  useEffect,
  useLayoutEffect,
...
/** Imagen satelital pública (mismo enfoque que el spec original con MapLibre + ESRI). */
const MAPLIBRE_ESRI_STYLE: StyleSpecification = {
  version: 8,
  sources: {
    esri: {
      type: "raster",
      tiles: [ESRI_TILE_TEMPLATE],
      tileSize: 256,
      attribution:
...
export default function MapCanvasMapLibre(props: MapCanvasProps) {
  return <MapLibreTerrainMap {...props} />;
}

function MapLibreTerrainMap({
  vertices,
  isClosed,
  areaM2,
  subLayers = [],
  ...
  useEffect(() => {
    ...
    const pad: maplibregl.PaddingOptions = {
      top: 56,
      right: 56,
      bottom: 56,
      left: 56,
  ...
  return (
    ...
    >
      <Map
        ...
        attributionControl={minimalChrome ? false : { compact: true }}
        reuseMaps
        onLoad={onMapLoad}
